...
const MyJobs = () => {
  ...
  useEffect(() => {
    try {
      axios.get("http://localhost:5000/api/v1/jobs/getmyjobs", { withCredentials: true }).then((res) => {
  ...
  const handleUpdateJob = async (jobId) => {
    ...
    await axios.put(`http://localhost:5000/api/v1/jobs/update/${jobId}`, updatedJob, { withCredentials: true }).then((res) => {
  ...
  const handleDeleteJob = async (jobId) => {
    const deletedJob = myJobs.find((job) => job._id === jobId)
